...
public class plot : MonoBehaviour {
	...
	void Start () {
		...
		plot1.transform.position = new Vector3 (0, 0, 2.0f);
		plot2.transform.position = new Vector3 (-2, 0, 0.5f);
		plot3.transform.position = new Vector3 (2.5f, 0, 0.5f);
	...
	void setupPlot1(GameObject plot)
	{
		for (int i=0; i< num_points; i++) {
			...
			cube.transform.position = new Vector3 (Random.Range(-1.0f, 1.0f), Random.Range(-1.0f, 1.0f), Random.Range(-0.3f, 0.3f));
	...
	void setupPlot2(GameObject plot)
	{
		for (int i=0; i< num_points; i++) {
			...
			cylinder.transform.position = new Vector3 (Random.Range(-1.0f, 1.0f), -1.0f, Random.Range(-1.0f, 1.0f));
			cylinder.transform.localScale = new Vector3 (0.1f, Random.Range(0.0f, 1.2f), 0.1f);
	...
	void setupPlot3(GameObject plot)
	{
		...
		for (int x = 0; x < num_points/10; x++) {
			for(int z = 0; z < num_points/10; z++)
			{
				...
				float xt = x * 2.0f/10.0f -1;
				float zt = z * 2.0f/10.0f -1;
